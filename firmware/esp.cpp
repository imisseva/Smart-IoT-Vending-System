...
int consecutiveOutliers = 0;          // Đếm số lần liên tiếp phát hiện khoảng cách tăng (nhiễu)
int consecutiveCupRemoved = 0;        // Đếm số lần liên tiếp phát hiện khoảng cách trống (mất ly)
  ...
  if (isDispensing) {
    if (now - lastDispensingMeasure >= 150) {
      ...
      if (isSensorReady) {
        ...
        if (measure.RangeStatus == 0) { // Sửa thành == 0 để đảm bảo kết quả đo cực kỳ chính xác
          float dist = measure.RangeMilliMeter / 10.0; // cm
          
          if (dist > 0 && dist <= MAX_PHYSICAL_DISTANCE) {
            // Kiểm tra an toàn rút ly nước: Nếu khoảng cách tiệm cận emptyDistance hoặc vượt quá đáy cốc
            if (dist >= emptyDistance - 1.0 || dist > cupBaseDistance + 2.0) {
              consecutiveCupRemoved++;
              if (consecutiveCupRemoved >= 4) { // Khoảng 600ms liên tiếp mất ly
                digitalWrite(currentActivePump, LOW);
                isDispensing = false;
                Serial.println("=== NGẮT BƠM KHẨN CẤP: PHÁT HIỆN LY BỊ RÚT KHỎI KHAY HỨNG! ===");
                guiBaoCaoRot(dist, 0.0, "CUP_REMOVED");
                currentActivePump = -1;
                consecutiveCupRemoved = 0;
                return;
              }
            } else {
              consecutiveCupRemoved = 0;
            ...
            // Xử lý mức nước và bộ lọc chống kẹt khóa mực nước
            if (dist <= cupBaseDistance + 0.5) {
              if (dist <= minDistanceMeasured + 0.5) {
                ...
                consecutiveOutliers++;
                Serial.print("[Nhiễu bọt] Phát hiện khoảng cách tăng: ");
                Serial.print(dist);
                Serial.print(" cm (Lần ");
                Serial.print(consecutiveOutliers);
                Serial.println(")");

                // Cơ chế tự động hồi phục nhiễu
                if (consecutiveOutliers >= 4) { // Nhiễu liên tục 600ms -> cập nhật giá trị mới
                  minDistanceMeasured = dist;
                  distance = minDistanceMeasured;
                  consecutiveOutliers = 0;
                  Serial.println(" ==> Tự động phục hồi bộ lọc: Đã cập nhật mốc khoảng cách mới.");
                } else {
                  distance = minDistanceMeasured; // Giữ nguyên khoảng cách nhỏ nhất an toàn cũ
                }
              }
            }
          }
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      if (httpCode > 0) {
        ...
        if (payload.startsWith("POUR_COCA_") || payload.startsWith("POUR_PEPSI_")) {
          ...
          // 0. Gửi ngay phản hồi ACK để báo Backend xóa lệnh an toàn
          ...
          for (int i = 0; i < 3; i++) {
            VL53L0X_RangingMeasurementData_t measure;
            sensor.rangingTest(&measure, false);
            if (measure.RangeStatus == 0) { // Sửa thành == 0
          ...
          // Gửi phản hồi ACK báo Backend xóa lệnh nhả ly
          guiBaoCaoRot(distance != -1 ? distance : emptyDistance, 0.0, "ACK");
